...
const hsheaderArr = [], hsbodyArr = [], hsurlArr = [], playurlArr = [],
    playheaderArr = [], playbodyArr = []
...
!(async () => {
  ...
  for (let i = 0; i < hsheaderArr.length; i++) {
    if (hsheaderArr[i]) {
      ...
      $.index = i + 1;
      console.log(`\n开始【抖音火山版${$.index}】`)
      console.log(`11111111111111共${hsurl}个账号----------------\n`)
      console.log(`22222222222222共${playheader}个账号----------------\n`)
...
function GetCookie() {
  ...
  if ($request && $request.url.indexOf("reaction/_play") >= 0) {
    ...
    if (playurl) {
      $.setdata(playurl, `playurl${status}`)
    }
    ...
    if (playbody) {
      $.setdata(playbody, `playbody${status}`)
    }
...
async function app_alert_check() {
  ...
  let check_url = 'https://ichannel.snssdk.com/service/2/app_alert_check/?'
      + iid
      + '&ac=WIFI&timezone=8&app_name=live_stream&channel=App%20Store&device_platform=iphone&'
      + idfa + '&' + vid
      + '&is_upgrade_user=0&app_verison_minor=10080507&version_code=10.8.5&'
      + device_id + '&os_version=13.3&' + aid + '&' + mcc_mnc
    ...
    $.get(userinfo_url, async (error, response, data) => {
      try {
        ...
        if (result.status_code == 0) {
          console.log('现有火苗：' + result.data.flame_left + '可兑换为：'
              + ((result.data.flame_left / 30000).toFixed(1)) + '元 现有余额：'
              + result.data.can_with_draw_money + '元')
          console.log('今日领取火苗' + result.data.td_flame_count)
          message += '今日领取火苗' + result.data.td_flame_count + ' 现有火苗'
              + result.data.flame_left + ' 可兑换为' + ((result.data.flame_left
                  / 30000).toFixed(1)) + '元 现有余额'
              + result.data.can_with_draw_money + '元\n'
        } else {
          console.log('👀我也不知道\n')
          message += '👀我也不知道\n'
  ...
  return new Promise((resolve) => {
    ...
    $.post(sign_inurl, async (error, response, data) => {
      try {
        ...
        if (result.status_code == 0) {
          console.log(
              '🎈' + '签到成功，获得' + result.data.task_done_award.flame_amount
              + '\n')
  ...
  return new Promise((resolve) => {
    ...
    $.post(ad_url, async (error, response, data) => {
      try {
        ...
        if (result.status_code == 0) {
          console.log(
              '🎈成功，获得' + result.data.task_done_award.flame_amount + '\n')
...
async function ck() {
  ...
  $.log(
      'hsheaser:' + hsheader.replace("{", `"{x-common-params-v2": "${hsurl}"`))
}

//hotsoonfeed
async function hotsoonfeed() {
  let new_time = Math.round(new Date().getTime() / 1000).toString();
  playheader = playheader.replace(/X-Khronos":"\d{10}/,
      `X-Khronos":"${new_time}`)
  return new Promise((resolve) => {
    let hotsoonfeed_url = {
    ...
    $.get(lottery_main_url, async (error, response, data) => {
      try {
        ...
        if (result.status_code == 0) {
          ...
          console.log(
              '增加抽奖次数' + task.task_current + '/' + task.task_total + '\n')
  ...
  return new class {
    ...
    writedata() {
      if (this.isNode()) {
        ...
        s ? this.fs.writeFileSync(t, r) : i ? this.fs.writeFileSync(e, r)
    ...
    lodash_set(t, e, s) {
      return Object(t) !== t ? t : (Array.isArray(e) || (e = e.toString().match(
          /[^.[\]]+/g) || []), e.slice(0, -1).reduce(
    ...
    initGotEnv(t) {
      this.got = this.got ? this.got : require(
          ...
          : new this.cktough.CookieJar, t && (t.headers = t.headers ? t.headers
    ...
    get(t, e = (() => {
      ...
      && (t.headers = t.headers || {}, Object.assign(t.headers,
          {"X-Surge-Skip-Scripting": !1})), $httpClient.get(t, (t, s, i) => {
        !t && s && (s.body = i, s.statusCode = s.status), e(t, s, i)
      })) : this.isQuanX() ? (this.isNeedRewrite && (t.opts = t.opts
          || {}, Object.assign(t.opts, {hints: !1})), $task.fetch(t).then(t => {
        const {statusCode: s, statusCode: i, headers: r, body: o} = t;
        e(null, {status: s, statusCode: i, headers: r, body: o}, o)
      }, t => e(t))) : this.isNode() && (this.initGotEnv(t), this.got(t).on(
          "redirect", (t, e) => {
            try {
    ...
    post(t, e = (() => {
      ...
      || this.isLoon()) {
        ...
        && (t.headers = t.headers || {}, Object.assign(t.headers,
            {"X-Surge-Skip-Scripting": !1})), $httpClient.post(t, (t, s, i) => {
          !t && s && (s.body = i, s.statusCode = s.status), e(t, s, i)
        });
    ...
    time(t) {
      ...
      for (let s in e) {
        new RegExp("(" + s + ")").test(t) && (t = t.replace(
            RegExp.$1, 1 == RegExp.$1.length ? e[s] : ("00" + e[s]).substr(
                ("" + e[s]).length)));
      }
      return t
    }
    ...
    msg(e = t, s = "", i = "", r) {
      const o = t => {
        ...
        if ("string" == typeof t) {
          return this.isLoon() ? t : this.isQuanX()
              ? {"open-url": t} : this.isSurge() ? {url: t} : void 0;
        }
        if ("object" == typeof t) {
          if (this.isLoon()) {
      ...
      if (this.isMute || (this.isSurge() || this.isLoon() ? $notification.post(
          e, s, i, o(r)) : this.isQuanX() && $notify(e, s, i,
          o(r))), !this.isMuteLog) {
        let t = ["",
          "==============\ud83d\udce3\u7cfb\u7edf\u901a\u77e5\ud83d\udce3=============="];
    ...
    logErr(t, e) {
      ...
      s ? this.log("", `\u2757\ufe0f${this.name}, \u9519\u8bef!`, t.stack)
